...
func (c ScooterClient) Release(scooterId uuid.UUID, location dto.Location) (scooterReleased bool) {
	marshalled, _ := json.Marshal(location)

	url := fmt.Sprintf("http://localhost:8080/api/scooters/%s/release", scooterId)
	req, _ := http.NewRequest(http.MethodPost, url, bytes.NewReader(marshalled))
